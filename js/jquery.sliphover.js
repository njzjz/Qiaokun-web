...
(function($, window, document, undefined) {
    ...
    SlipHover.prototype = {
        ...
        createOverlay: function(instance, direction, $element) {
            ...
            /*
            *增加漂浮容器控制方式2017-06-05 WH
            */
            var captionCell = this.settings.captionCell,$captionCell,captionEleHtml,
                captionHtml = $element.attr(instance.settings.caption);//文本方式
                //容器方式
                $element.siblings(captionCell).length ? $captionCell = $element.siblings(captionCell) : $captionCell = $element.find(captionCell);
                captionEleHtml = $captionCell.prop("outerHTML");
               
                if($captionCell.length) captionHtml = captionEleHtml;
            // console.log(captionEleHtml);
            //改变浮层的内容
